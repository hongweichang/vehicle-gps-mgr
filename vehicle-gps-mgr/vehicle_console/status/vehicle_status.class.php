...
class Vehicle_status extends BASE {
	...
	private function retrieve_data() {
		...
		$this->sql = sprintf ( "select vm.*,ge.gps_number from %s vm left join gps_equipment ge on vm.gps_index_id = ge.id where vm.id = %d", $this->tablename, $this->id );
	 ...
	 * 从XML中解析彭总那获得的地址信息
	 */
	private function parse_location_dis($location) {
		$begin = stripos ( $location, "<RECORD" );
		$result = substr ( $location, $begin + 7 );
		$next_begin = stripos ( $result, ">" );
		
		$end = stripos ( $result, "</RECORD>" );
		$length = $end - $next_begin;
		
		$result = substr ( $result, $next_begin + 1, $length - 1 );
		
		//		$length = stripos ( $result, "</RECORD>" );
	...
	function gps_status_boolean($gps_status) {
		...
		if ($gps_status == null or $gps_status == 0)
			return "离线";
		if ($gps_status == 1)
			return "在线";
	}
	...
	function get_lon_lat() {
		$company_id = get_session ( "company_id" ); //获取公司ID
		$this->sql = "select cur_longitude,cur_latitude from " . $this->tablename . " where company_id=" . $company_id;
		$this->data_list = $GLOBALS ['db']->query ( $this->sql );
		return $this->data_list;
	}
