...
$(document).ready(
		function() {
			...
			var myDate = new Date();
			if (isInherit) {
				var startTime = $(window.parent.document).find(
						"#inquire_startTime").attr("value");
				var endTime = $(window.parent.document)
						.find("#inquire_endTime").attr("value");
...
function play_trace() {
	...
	if (history_track_frame.state === "stop") {
		...
		// 获取车牌号、GPS编号
		$.get("index.php?a=504&vehicle_id=" + history_track_frame.vehicle_id,
				function(data) {
...
var trace_map_type = "google";
// 切换地图
function trace_change_map() {
	if(history_track_frame.state != "stop"){
		history_track_frame.end_history_line();
	...
	if ("51ditu" == trace_map_type) {
		...
		document.getElementById("history_track_frame").src = "inquire/templates/history_track_google.html";
	} else {
		trace_map_type = "51ditu";

		document.getElementById("history_track_frame").src = "inquire/templates/history_track_map.html";
	}
}
