...
/**
 * 刷新状态(注：刷新不同操作，例：刷新公司车辆) 0 刷新公司所有车辆 1 刷新选择车辆 2 停止刷新
 */
var refresh_state = 0;
/**
 * 刷新最新车辆集合 例:1,2,3,4,5 定位最新车辆集合
 */
...
function getPoi(name) {
	if (name != null && name != "") {
		$.ajax({
			type : "POST",
			url : window.parent.host + "/index.php?a=105&name="
					+ encodeURI(name) + "&longitude=" + poi.getLongitude()
			...
			success : function(data) {
				...
				if (data == 1) { // 成功
					// 创建标签点
					var company_text = new LTMapText(new LTPoint(poi
							.getLongitude(), poi.getLatitude()));
...
function position_add_showinfo(obj) {
	...
	function show_maker_info() {
		...
		$
				.get(
						"/index.php?a=108&longitude=" + longitude
								+ "&latitude=" + latitude,
						function(positions) {
							...
							info
									.setTitle("<div style='font-weight:700;font-size:12px;'>北京龙菲业</div>");
							info
									.setLabel("<div><div class='lable'><div class='lable_title'>联系人：</div><div class='lable_content'>"
											+ contact
											+ "</div></div>"
											+ "<div class='lable'><div class='lable_title'>邮编：</div><div class='lable_content'>"
											+ zipcode
											+ "</div></div>"
											+ "<div class='lable'><div class='lable_title'>电话：</div><div class='lable_content'>"
											+ tel
											+ "</div></div>"
											+ "<div class='lable'><div class='lable_title'>传真：</div><div class='lable_content'>"
											+ fax
											+ "</div></div>"
											+ "<div class='lable'><div class='lable_title'>邮箱：</div><div class='lable_content'>"
											+ mail
											+ "</div></div>"
											+ "<div class='lable'><div class='lable_title'>网址：</div><div class='lable_content'>"
											+ url
											+ "</div></div>"
											+ "<div class='lable'><div class='lable_title'>地址：</div><div class='lable_content'>"
											+ address
											+ "</div></div>"
											+ "<div class='lable'><div class='lable_title'>编辑：</div><div class='lable_content'><a href='javascript:modify_position("
											+ id
											+ ")'>修改</a></div></div>"
											+ "<div class='lable'><div class='lable_title'>删除：</div><div class='lable_content'><a href='javascript:delete_position("
											+ id
											+ ")'>删除</a></div></div>"
											+ "</div> ");
...
function update_position(name, id) {
	$.get("/index.php?a=109&position_id=" + id + "&name=" + encodeURI(name),
			function(data) {
				if ("ok" == data) {
					alert("修改成功");
...
function show_all_vehicle_location(data, length, run_index) {
	...
	for ( var i = 0; i < length; i++) {
		...
		point_longitude = formart_lnglat(data[0]['cur_longitude']); // 当前经度
		point_latitude = formart_lnglat(data[0]['cur_latitude']); // 当前纬度
 ...
 * 
 * @param {Object}
 *            longitudeArray 车辆经度点数组
 * @param {Object}
 *            latitudeArray 车辆纬度点数组
 * @param {Object}
 *            points 车辆点数组
 */
...
function get_data(data) {
	if (data != null) {
		...
		var context = "<div class='content_div'><div class='title'>GPS编号：</div>"
				...
				"<div class='real_time_monitor'><a href='javascript:refresh_vehicle_position("
				+ vehicle_id
				+ ");save_cur_monitor_vehicles("
				+ vehicle_id
				+ ")'>实时监控</a></div></div>";
