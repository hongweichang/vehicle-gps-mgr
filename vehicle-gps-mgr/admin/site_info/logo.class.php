...
class Logo{
	...
	var $ext;// 文件扩展名
	var $thumb = 0;// 是否生成缩略图
	var $thumbWidth = 80;// 缩略图宽
	var $thumbHeight = 80;// 缩略图高
	var $thumbPrefix = "logo_";// 缩略图前缀
	var $errno;// 错误代号
	...
	var $returninfo= array();// 每个文件返回信息
		...
		// @param $overwriet 是否覆盖 1 允许覆盖 0 禁止覆盖
			...
			function Logo($savePath, $fileFormat='',$maxSize = 0, $overwrite = 0) {
		...
		// 得到错误信息
			function errmsg(){
				...
				if ($this->errno == 0)
					...
					return $uploadClassError[$this->rrno];
			}
}
?>