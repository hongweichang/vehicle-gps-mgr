...
//				$searchstr = $db->prepare_value($searchstr,$type);
			...
			foreach($rtn as	$key=>$rtn_driver)
			{
				...
				$responce->rows[$key]['cell']=array($rtn_driver['name'],$rtn_driver['driving_licence_id'],$sex[$rtn_driver['sex']],$rtn_driver['birthday'],$rtn_driver['career_time'],$rtn_driver['job_number'],$driving_type[$rtn_driver['driving_type']],$rtn_driver['mobile'],$driving_state[$rtn_driver['driving_state']],$rtn_driver['phone_email'],$rtn_driver['address']);
			}
	...
	case "edit_data":
		...
		switch($oper)
		{
			...
			case "edit":
				...
				$parms["birthday"]				= $GLOBALS['db']->prepare_value($_REQUEST["birthday"],"VARCHAR");
				//取的 session
				$parms["company_id"]			= $GLOBALS['db']->prepare_value(get_session("company_id"),"VARCHAR");
				$parms["career_time"]			= $GLOBALS['db']->prepare_value($_REQUEST["career_time"],"VARCHAR");
				$parms["job_number"]			= $GLOBALS['db']->prepare_value($_REQUEST["job_number"],"VARCHAR");
				$parms["driving_type"]			= $_REQUEST["driving_type"];
				$parms["mobile"]				= $GLOBALS['db']->prepare_value($_REQUEST["mobile"],"VARCHAR");
